...
var feesFmt = `{"type": "spend_account", "asset_id": "` + consensus.NativeAssetID.String() + `", "amount":%s, "account_id": "%s"}`
...
func SendReq(method string, args []string) (interface{}, bool) {
	...
	case BuildMulTx:
		...
		fees += fmt.Sprintf(feesFmt, gasAmount, accountInfo) + ","
		switch buildType {
		case BuildIssue:
...
func Sendbulktx(threadTxNum int, txAssetNum string, sendAcct string, sendasset string, controlPrograms []string, txidChan chan string, index *uint64) {
	...
	for i := 0; i < threadTxNum; i++ {
		...
		param := []string{sendAcct, sendasset, txAssetNum, receiver, cfg.BuildType}
		if mulOutput > 0 {
