...
func init() {
	...
	sendTxCmd.PersistentFlags().IntVar(&assetNum, "assetnum", 100000000, "Number of transactions asset,unit: uny")
...
var sendTxCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		txAssetNum := fmt.Sprintf("%d", assetNum)
		fmt.Println("*****************send tx start*****************")
		// send asset to account
		index := uint64(0)
		for i := 0; i < thdNum; i++ {
			go Sendbulktx(thdTxNum, txAssetNum, sendAcct, sendasset, controlPrograms, txidChan, &index)
		}
