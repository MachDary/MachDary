...
import (
	...
	"github.com/MachDary/MachDary/net/p2p"
	"github.com/MachDary/MachDary/net/p2p/discover"
	core "github.com/MachDary/MachDary/protocol"
	"github.com/MachDary/MachDary/protocol/bc"
	"github.com/MachDary/MachDary/protocol/bc/types"
	"github.com/MachDary/MachDary/version"
...
// Chain is the interface for Chain core
...
func (sm *SyncManager) handleTransactionMsg(peer *peer, msg *TransactionMessage) {
	...
	acceptable, height, gasStatus, err := sm.chain.ValidateTx(tx)

	var isOrphan = false
	if acceptable {
		isOrphan, err = sm.chain.ProcessTransaction(tx, err != nil, height, gasStatus.AssetValue)
	}

	if err != nil && isOrphan == false {
		sm.peers.addBanScore(peer.ID(), 10, 0, "fail on process tx transaction")
		return
	}
}
