...
func TestOverflow(t *testing.T) {
	...
	newTx := func(inputs []uint64, outputs []uint64) *bc.Tx {
		...
		for _, amount := range inputs {
			txInput := types.NewSpendInput(nil, *sourceID, *consensus.NativeAssetID, amount, 0, ctrlProgram)
			txInputs = append(txInputs, txInput)
		}

		for _, amount := range outputs {
			txOutput := types.NewTxOutput(*consensus.NativeAssetID, amount, ctrlProgram)
...
func TestTxValidation(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			f: func() {
				addCoinbase(consensus.NativeAssetID, 100000, nil)
			},
			err: nil,
		},
		{
		...
		},
		{
			...
			f: func() {
				...
				addCoinbase(consensus.NativeAssetID, 100000, arbitrary)
			},
...
func TestTimeRange(t *testing.T) {
	...
	tx := types.MapTx(&types.TxData{
		...
		Outputs: []*types.TxOutput{
			types.NewTxOutput(*consensus.NativeAssetID, 1, []byte{0x6a}),
		},
	})
...
func mockCoinbaseTx(amount uint64) *bc.Tx {
	...
	return types.MapTx(&types.TxData{
		...
		Outputs: []*types.TxOutput{
			types.NewTxOutput(*consensus.NativeAssetID, amount, cp),
		},
	})
}
...
func mockGasTxInput() *types.TxInput {
	cp, _ := vmutil.DefaultCoinbaseProgram()
	return types.NewSpendInput([][]byte{}, *newHash(8), *consensus.NativeAssetID, 100000000, 0, cp)
}
