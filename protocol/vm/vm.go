...
func Verify(context *Context, gasLimit int64) (res []byte, gasLeft int64, err error) {
	defer func() {
		if r := recover(); r != nil {
	...
	if context.VMVersion != 1 {
		return nil, gasLimit, ErrUnsupportedVM
	}
	...
	for i, arg := range args {
		...
		if err != nil {
			return nil, vm.runLimit, errors.Wrapf(err, "pushing initial argument %d", i)
		}
	}
	...
	return res, vm.runLimit, wrapErr(err, vm, args)
}
