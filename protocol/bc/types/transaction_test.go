	...
	cases := []struct {
		...
		},
		{
			tx: NewTx(TxData{
				...
				Inputs: []*TxInput{
					...
					NewSpendInput([][]byte{[]byte("arguments3"), []byte("arguments4")}, testutil.MustDecodeHash("fad5195a0c8e3b590b86a3c0a95e7529565888508aecca96e9aeda633002f409"), *consensus.NativeAssetID, 254354, 3, []byte("spendProgram")),
				},
				Outputs: []*TxOutput{
			...
			hex: strings.Join([]string{
				...
				consensus.NativeAssetID.String(), // input 1: assetID
		...
		},
		{
			tx: NewTx(TxData{
				...
				Outputs: []*TxOutput{
					NewTxOutput(*consensus.NativeAssetID, 254354, []byte("true")),
					NewTxOutput(*consensus.NativeAssetID, 254354, []byte("false")),
				},
			}),
			hex: strings.Join([]string{
				"07",                 // serflags
				...
				"01",                 // output 0: asset version
				"29",                 // output 0: serialization length
				consensus.NativeAssetID.String(), // output 0: assetID
				"92c30f",   // output 0: amount
				"01",       // output 0: version
				"04",       // output 0: control program length
				"74727565", // output 0: control program
				...
				consensus.NativeAssetID.String(), // output 1: assetID
