...
var testTxs = []*types.Tx{
	types.NewTx(types.TxData{
		...
		Inputs: []*types.TxInput{
			types.NewSpendInput(nil, bc.NewHash([32]byte{0x01}), *consensus.NativeAssetID, 1, 1, []byte{0x51}),
		},
	...
	types.NewTx(types.TxData{
		...
		Outputs: []*types.TxOutput{
			types.NewTxOutput(*consensus.NativeAssetID, 1, []byte{0x6b}),
		},
	}),
	types.NewTx(types.TxData{
		SerializedSize: 150,
...
func (s *mockStore) DB() dbm.DB { return nil }
