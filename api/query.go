...
func (a *API) decodeRawTransaction(ctx context.Context, ins struct {
	...
	totalInputAsset := uint64(0)
	totalOutputAsset := uint64(0)
	for _, input := range tx.Inputs {
		if input.AssetID.String() == consensus.NativeAssetID.String() {
			totalInputAsset += input.Amount
		}
	}

	for _, output := range tx.Outputs {
		if output.AssetID.String() == consensus.NativeAssetID.String() {
			totalOutputAsset += output.Amount
		}
	}

	tx.Fee = int64(totalInputAsset) - int64(totalOutputAsset)
	return NewSuccessResponse(tx)
}
