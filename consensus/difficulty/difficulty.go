...
func CalcTargetBits(difficulty uint64) uint64 {
	denominator := new(big.Int).SetUint64(difficulty)
	difficultyNum := new(big.Int).Div(oneLsh256, denominator)
	difficultyNum = new(big.Int).Sub(difficultyNum, bigOne)
	return BigToCompact(difficultyNum)
}
...
func CheckProofOfWork(hash, seed *bc.Hash, bits uint64) bool {
	...
	//fmt.Println(hex.EncodeToString(b32[:]))
	//bs, _ := scrypt.Key(hash.Bytes(), seed.Bytes(), 1024, 1, 1, 32)
	//copy(b32[:], bs)
	//fmt.Println(hex.EncodeToString(b32[:]))
	compareHash := bc.NewHash(b32)
	//fmt.Println(compareHash.String())
