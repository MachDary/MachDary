...
func Build(ctx context.Context, tx *types.TxData, actions []Action, maxTime time.Time, timeRange uint64, chain *protocol.Chain) (*Template, error) {
...
func checkBalance(tx *types.TxData) error {
	...
	for assetId, amt := range assetMap {
		// NativeAsset could be left as gas fee
		if amt > 0 && assetId != *consensus.NativeAssetID {
