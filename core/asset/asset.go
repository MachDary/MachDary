...
import (
	...
	"github.com/MachDary/MachDary/basis/crypto/ed25519"
...
func initNativeAsset() {
	...
	definitionBytes, _ := serializeAssetDef(consensus.NativeAssetDefinitionMap)
	DefaultNativeAsset = &Asset{
		Signer:            signer,
		AssetID:           *consensus.NativeAssetID,
		Alias:             &alias,
		VMVersion:         1,
		DefinitionMap:     consensus.NativeAssetDefinitionMap,
...
var (
	...
	ErrInternalAsset  = errors.New(strings.ToLower(consensus.NativeAssetAlias) + " has been defined as the internal asset")
...
func (reg *Registry) Define(xpubs []chainkd.XPub, quorum int, definition map[string]interface{}, alias string, issuanceProgram chainjson.HexBytes) (*Asset, error) {
	...
	if alias == consensus.NativeAssetAlias {
...
func (reg *Registry) GetAliasByID(id string) string {
	//nativeAsset
	if id == consensus.NativeAssetID.String() {
		return consensus.NativeAssetAlias
	}
...
func (reg *Registry) UpdateAssetAlias(id, newAlias string) error {
	...
	if oldAlias == consensus.NativeAssetAlias || newAlias == consensus.NativeAssetAlias {
