...
func TestRPCCallJSON(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		...
		if !strings.HasPrefix(req.Header.Get("User-Agent"), strings.Title(consensus.NativeChainName) + "; ") {
			t.Errorf("got=%s; want prefix='%s; '", req.Header.Get("User-Agent"), strings.Title(consensus.NativeChainName))
		}
