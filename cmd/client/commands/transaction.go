...
import (
	...
	"github.com/MachDary/MachDary/api"
	...
	"github.com/MachDary/MachDary/util"
...
func init() {
	...
	buildTransactionCmd.PersistentFlags().StringVarP(&gasAmount, "gas", "g", "20000000", "gas of this transaction")
...
var buildIssueReqFmt = `
	{"actions": [
		{"type": "spend_account", "asset_id": "` + consensus.NativeAssetID.String() + `", "amount":%s, "account_id": "%s"},
...
var buildIssueReqFmtByAlias = `
	{"actions": [
		{"type": "spend_account", "asset_alias": "` + consensus.NativeAssetAlias + `", "amount":%s, "account_alias": "%s"},
...
var buildTransactionCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		case "issue":
			if alias {
				buildReqStr = fmt.Sprintf(buildIssueReqFmtByAlias, gasAmount, accountInfo, assetInfo, amount, assetInfo, amount, address)
				break
			}
			buildReqStr = fmt.Sprintf(buildIssueReqFmt, gasAmount, accountInfo, assetInfo, amount, assetInfo, amount, address)
		case "spend":
			if alias {
				buildReqStr = fmt.Sprintf(buildSpendReqFmtByAlias, gasAmount, accountInfo, assetInfo, amount, accountInfo, assetInfo, amount, receiverProgram)
				break
			}
			buildReqStr = fmt.Sprintf(buildSpendReqFmt, gasAmount, accountInfo, assetInfo, amount, accountInfo, assetInfo, amount, receiverProgram)
		case "retire":
			if alias {
				buildReqStr = fmt.Sprintf(buildRetireReqFmtByAlias, gasAmount, accountInfo, assetInfo, amount, accountInfo, assetInfo, amount, arbitrary)
				break
			}
			buildReqStr = fmt.Sprintf(buildRetireReqFmt, gasAmount, accountInfo, assetInfo, amount, accountInfo, assetInfo, amount, arbitrary)
		case "address":
			if alias {
				buildReqStr = fmt.Sprintf(buildControlAddressReqFmtByAlias, gasAmount, accountInfo, assetInfo, amount, accountInfo, assetInfo, amount, address)
				break
			}
			buildReqStr = fmt.Sprintf(buildControlAddressReqFmt, gasAmount, accountInfo, assetInfo, amount, accountInfo, assetInfo, amount, address)
		case "program":
			if alias {
				buildReqStr = fmt.Sprintf(buildControlProgramReqFmtByAlias, gasAmount, accountInfo, assetInfo, amount, accountInfo, assetInfo, amount, program)
				break
			}
			buildReqStr = fmt.Sprintf(buildControlProgramReqFmt, gasAmount, accountInfo, assetInfo, amount, accountInfo, assetInfo, amount, program)
		case "unlock":
			var err error
			usage := "Usage:\n  client build-transaction <accountID|alias> <assetID|alias> <amount> <outputID> -c <contractName>"
			baseCount := 4
