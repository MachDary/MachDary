...
import (
	...
	"github.com/MachDary/MachDary/core/pseudohsm"
	...
	"github.com/MachDary/MachDary/database/leveldb"
...
func BenchmarkChain_NormalTx_NoAsset_BASE(b *testing.B) {
	benchInsertChain(b, 1, 0, "")
}

func BenchmarkChain_5000NormalTx_NoAsset_BASE(b *testing.B) {
	benchInsertChain(b, 5000, 0, "")
}

func BenchmarkChain_5000NormalTx_1Asset_BASE(b *testing.B) {
	benchInsertChain(b, 5000, 1, "")
}

// standard Transaction
func BenchmarkChain_NormalTx_NoAsset_P2PKH(b *testing.B) {
	benchInsertChain(b, 1000, 0, "P2PKH")
}

func BenchmarkChain_NormalTx_1Asset_P2PKH(b *testing.B) {
	benchInsertChain(b, 1000, 1, "P2PKH")
}

func BenchmarkChain_NormalTx_NoAsset_P2SH(b *testing.B) {
	benchInsertChain(b, 100, 0, "P2SH")
}

func BenchmarkChain_NormalTx_1Asset_P2SH(b *testing.B) {
	benchInsertChain(b, 100, 1, "P2SH")
}

func BenchmarkChain_NormalTx_NoAsset_MultiSign(b *testing.B) {
	benchInsertChain(b, 100, 0, "MultiSign")
}

func BenchmarkChain_NormalTx_1Asset_MultiSign(b *testing.B) {
...
func GenerateBaseUtxos(num int, amount uint64, ctrlProg *account.CtrlProgram) []*account.UTXO {
	utxos := []*account.UTXO{}
	for i := 0; i < num; i++ {
		utxo := MockSimpleUtxo(uint64(i), consensus.NativeAssetID, amount, ctrlProg)
...
func CreateTxBuilder(baseUtxo *account.UTXO, nativeAssetServiceFlag bool, signer *signers.Signer) (*txbuilder.TemplateBuilder, error) {
	...
	// if the nativeAsset is the service charge, didn't need to add the output
	if nativeAssetServiceFlag {
...
func BuildTx(baseUtxo *account.UTXO, otherUtxos []*account.UTXO, signer *signers.Signer) (*txbuilder.Template, error) {
	nativeAssetServiceFlag := false
	if otherUtxos == nil || len(otherUtxos) == 0 {
		nativeAssetServiceFlag = true
	}

	tplBuilder, err := CreateTxBuilder(baseUtxo, nativeAssetServiceFlag, signer)
	if err != nil {
		return nil, err
	}
...
func MockTxsP2PKH(keyDirPath string, testDB dbm.DB, txNumber, otherAssetNum int) ([]*types.Tx, error) {
	...
	for i := 0; i < txNumber; i++ {
		...
		utxo := MockSimpleUtxo(0, consensus.NativeAssetID, 1000000000, controlProg)
