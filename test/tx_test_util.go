...
func (g *TxGenerator) assetAmount(assetAlias string, amount uint64) (*bc.AssetAmount, error) {
	if assetAlias == consensus.NativeAssetAlias {
		a := &bc.AssetAmount{
			Amount:  amount,
...
func txFee(tx *types.Tx) uint64 {
	...
	for _, input := range tx.Inputs {
		if input.AssetID() == *consensus.NativeAssetID {
...
func CreateCoinbaseTx(controlProgram []byte, height, txsFee uint64) (*types.Tx, error) {
	...
	if err := builder.AddOutput(types.NewTxOutput(*consensus.NativeAssetID, coinbaseValue, controlProgram)); err != nil {
		return nil, err
	}
...
func CreateTxFromTx(baseTx *types.Tx, outputIndex uint64, outputAmount uint64, ctrlProgram []byte) (*types.Tx, error) {
	...
	output := types.NewTxOutput(*consensus.NativeAssetID, outputAmount, ctrlProgram)
	builder := txbuilder.NewBuilder(time.Now())
