...
func DefaultP2PConfig() *P2PConfig {
	return &P2PConfig{
		ListenAddress:    "tcp://0.0.0.0:60516",
...
func DefaultDataDir() string {
	...
	home := homeDir()
	if home == "" {
		return "./." + strings.ToLower(consensus.NativeChainName)
	}
	switch runtime.GOOS {
	case "darwin":
		return filepath.Join(home, "Library", strings.Title(consensus.NativeChainName))
	case "windows":
		return filepath.Join(home, "AppData", "Roaming", strings.Title(consensus.NativeChainName))
	default:
		return filepath.Join(home, "." + strings.ToLower(consensus.NativeChainName))
	}
}
