...
BUILD_FLAGS := -ldflags "-X github.com/MachDary/MachDary/version.GitCommit=`git rev-parse HEAD`"

CHAIN_NAME := MachDary

MINER_BINARY := $(CHAIN_NAME)-miner-$(GOOS)_$(ARCH)

SERVER_BINARY := $(CHAIN_NAME)-server-$(GOOS)_$(ARCH)

CLIENT_BINARY := $(CHAIN_NAME)-client-$(GOOS)_$(ARCH)
